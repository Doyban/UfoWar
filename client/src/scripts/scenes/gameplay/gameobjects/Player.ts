...
export default class Player extends Phaser.Physics.Arcade.Sprite
{
    ...
    private onSpaceDown()
    {
        let dome = this.scene.physics.add.sprite(this.x, this.y, "shipwear", "laserBlue3.png");
        dome.enableBody(true, dome.x, dome.y, true, true);
        dome.setScale(0.5, -0.5);
        dome.rotation = this.rotation;
        this.scene.physics.velocityFromRotation(this.rotation - 4.7, 500, dome.body.velocity);
        this.scene.events.emit(EventNames.BULLET, { x: dome.x, y: dome.y, rotation: dome.rotation });
