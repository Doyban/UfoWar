...
export default class GamePlayScene extends Phaser.Scene
{
    ...
    preload()
    {
        ...
        this.load.atlas("shipwear", "assets/images/shipwear.png", "assets/json/shipwear.json")
        this.load.atlas("gui", "assets/images/gui.png", "assets/json/gui.json")
        this.load.atlasXML("sheet", "assets/spritesheets/sheet.png", "assets/spritesheets/sheet.xml");
        this.load.atlasXML("explosion", "assets/spritesheets/spritesheet_regularExplosion.png", "assets/spritesheets/spritesheet_regularExplosion.xml");
    }
    ...
    addGameoverText()
    {
        this.gameOverText = this.add.text(+this.game.config.width / 2, 250, "Game Over");
    ...
    update(time, delta)
    {
        ...
        if (this.enemyBullets)
        {
            for (let i = 0; i < this.enemyBullets.length; i++)
            {
                ...
                if ((enemyBullet.x < 0 || enemyBullet.x > this.game.config.width) || (enemyBullet.y < 0 || enemyBullet.y > this.game.config.height))
                {
                    enemyBullet.destroy();
        ...
        if (this.playerBullets)
        {
            for (let i = 0; i < this.playerBullets.length; i++)
            {
                ...
                if ((playerBullet.x < 0 || playerBullet.x > this.game.config.width) || (playerBullet.y < 0 || playerBullet.y > this.game.config.height))
                {
        ...
        if (this.obstacles)
        {
            for (let i = 0; i < this.obstacles.length; i++)
            {
                ...
                if ((obstacle.x < 0 || obstacle.x > this.game.config.width) || (obstacle.y < 0 || obstacle.y > this.game.config.height))
                {
                    obstacle.destroy();
        ...
        if (this.player)
        {
            for (let i = 0; i < this.enemyBullets.length; i++)
            {
                ...
                if (this.player.x < enemyBullet.x + enemyBullet.width &&
        ...
        if (this.enemy)
        {
            for (let i = 0; i < this.playerBullets.length; i++)
            {
                ...
                if (this.enemy.x < playerBullet.x + playerBullet.width &&
    ...
    {
        let dome = this.physics.add.sprite(obj.x, obj.y, "shipwear", "laserBeige3.png");
        dome.enableBody(true, dome.x, dome.y, true, true);
        dome.setScale(0.5, -0.5);
        dome.rotation = obj.rotation
        this.physics.velocityFromRotation(dome.rotation - 4.7, 500, dome.body.velocity); // physics function to make movement from the current rotation of the object
    ...
    private onAddAstroid(obj)
    {
        ...
        this.physics.velocityFromRotation(obj.rotation, obj.speed, astroid.body.velocity);
        astroid.setDepth(1);
